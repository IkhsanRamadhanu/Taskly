...
const Login = () => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      navigate('/tasks'); // Gunakan navigate untuk navigasi
    } catch (err) {
      console.error(err);
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-200 p-6" style={{
      ...
      <form onSubmit={handleLogin} className="bg-white p-6 rounded-xl shadow-lg w-full max-w-[40rem] flex flex-col items-center border">
        ...
        <div className='w-full flex justify-between items-start gap-4'>
          <img src="/login-pict.png" alt="Login Vector" className=' w-64 h-64 w'/>
          <div className='w-full h-full flex flex-col justify-start'>
            <h2 className="text-2xl mb-1 font-semibold">Log In</h2>
            <div>
            ...
            <div className=' flex mt-4 justify-end gap-4 self-center'>
                ...
                <button type="button" className="border border-black text-black w-24 font-medium py-1 px-4 rounded hover:scale-105 active:scale-90 duration-300" onClick={() => navigate('/signup')}>
                Sign Up
                </button>
            </div>
          </div>
